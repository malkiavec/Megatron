...
Os dados são rotulados incluindo um `customer_id` de sua escolha (veja as restrições em [Como rotular dados](/docs/services/discovery/information-security.html#labeling)) no cabeçalho `X-Watson-Metadata` opcional. O {{site.data.keyword.discoveryshort}}  pode, então, excluí-lo por  ` customer_id `.
...
- eventos (` /v1/events `)
- documentos (` /v1/environments/ { /collections/ { /documents `)
- avisos (`/v1/environments/{environment_id}/collections/{collection_id}/notices`) Somente `notices` de ingestão são rotulados.
- Entidades do Knowledge Graph (` /v1/environments/ { /collections/ { /query_entities `)
- Relações de Gráficos de Conhecimento (` /v1/environments/ { /collections/ { /query_UNK `)
- dados de treinamento (` /v1/environments/ { /collections/ { /training_data `)
...
1. Atualize sua configuração de adaptador de saída do {{site.data.keyword.discoveryshort}} Data Crawler para incluir o `customer_id`. Consulte  [ Configurando o adaptador de saída ](/docs/services/discovery/data-crawler-discovery.html#output-adapter).
...
1. Use a operação `DELETE /v1/user_data` e forneça o `customer_id` dos dados que você deseja excluir. `DELETE /v1/user_data` exclui todos os dados associados a um `customer_id` específico dentro dessa instância de serviço, conforme especificado em [Métodos que suportam dados de identificação](/docs/services/discovery/information-security.html#pi_methods). Veja também a [Referência da API ![Ícone de link externo](../../icons/launch-glyph.svg "Ícone de link externo")](https://www.ibm.com/watson/developercloud/discovery/api/v1/curl.html#delete-user-data){: new_window}
