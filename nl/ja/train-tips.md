...
トレーニングされたコレクションは、自然言語照会の結果で `confidence` スコアを返します。この `confidence` 数値は、トレーニングされたモデルに比べて、結果がどのくらい関連しているのかに基づいて計算されます。 `confidence` スコアは `score` と同じでは**ありません**。 詳しくは、『[信頼度スコア](/docs/services/discovery/train-tooling.html#confidence)』を参照してください。  
