...
bash の POST /v1/environments メソッドを使用して環境を作成します。環境とは、すべての文書を箱に入れて保管する倉庫のようなものです。
次の例では、`my-first-environment` という名前の環境を作成します。
...
1.  **「用途 (Intended Use)」**で、ご使用のシステム (DEB、RPM、または ZIP) に適したダウンロード・リンクを選択し、Data Crawler をダウンロードします。
