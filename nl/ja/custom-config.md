...
`"exclude_tag_attributes" : 配列` - 変換によって削除される HTML 属性の名前の配列。その HTML 属性がどの HTML タグに含まれるのかは問いません。

#### keep_tag_attributes

`"keep_tag_attributes" : 配列` - 変換によって保持される HTML 属性の名前の配列。
...
{{site.data.keyword.discoveryshort}} は、{{site.data.keyword.nlushort}} エンリッチメントと要素の分類 (Element Classification) エンリッチメントの追加をサポートします。エンリッチする各フィールドは、`enrichments` 配列内のオブジェクトによって定義します。各エンリッチメント・オブジェクトには、`source_field`、`destination_field`、およびエンリッチメントを指定する必要があります。
