...
Die hier verwendeten Beispielabfragen wurden mit den {{site.data.keyword.discoveryshort}}-Tools erstellt. Wenn Sie stattdessen die API verwenden möchten, fügen Sie die Abfrageparameter zu Ihrem API-Aufruf hinzu. Weitere Informationen und Beispiele enthält der Abschnitt 'Abfragen' der [API-Referenz ![Symbol für externen Link](../../icons/launch-glyph.svg "Symbol für externen Link")](https://www.ibm.com/watson/developercloud/discovery/api/v1/#query-collection){: new_window}.
...
1.  [Starten Sie die {{site.data.keyword.discoveryshort}}-Tools](/docs/services/discovery/getting-started-tool.html#launch-the-tooling). Wählen Sie in der Anzeige **Daten verwalten** die Sammlung '{{site.data.keyword.IBM_notm}} Press Releases' aus.

1.  Prüfen Sie die Informationen, die Watson in Ihren aufbereiteten Dokumenten ermittelt hat.

    -  Bei **Allgemeine Stimmung** wird die prozentuale Aufschlüsselung der als positiv, neutral oder negativ gekennzeichneten Dokumente angezeigt, die von der Stimmungsanalyse erkannt wurden.
    -  Bei **Wichtigste Entitäten** werden Personen, Orte und Organisationen angezeigt, die durch die Aufbereitung der Entitätsextraktion in Ihren Dokumenten erkannt wurden.
    -  Bei **Inhaltshierarchie** werden die hierarchischen Taxonomien angezeigt, die von der Aufbereitung für die Kategorieklassifizierung in Ihren Dokumenten erkannt wurden.
    -  Bei **Zugehörige Konzepte** werden die Konzepte angezeigt, die von der Aufbereitung für das Konzepttagging in Ihren Dokumenten erkannt wurden.

         Klicken Sie auf einer beliebigen Karte auf **Im Schema anzeigen**, um die Aufbereitungen anzusehen, die diese Ergebnisse beinhalten.
         {: tip}
...
1.  Machen Sie sich mit dem Datenschema Ihrer Dokumente vertraut, indem Sie die Anzeige **Datenschema anzeigen** ansehen. Hier werden die Felder und Werte in Ihren konvertierten Dokumenten auf zwei Arten angezeigt, nämlich nach Dokument (**Dokumentansicht**) oder nach Feld (**Sammlungsansicht**). In der **Sammlungsansicht** werden alle Felder Ihrer Sammlung angezeigt.
    ...
    Klicken Sie auf die Schaltfläche **Datenschema anzeigen**. In der **Sammlungsansicht** können Sie unter `enriched_text` die Aufbereitungen untersuchen, die Sie mit der Datei für die **Standardkonfiguration** angewendet haben. Klicken Sie auf `categories`, `concepts`, `entities` und `sentiment`, um festzustellen, wie Ihre Sammlung mit Watson-Informationen aufbereitet wurde.
...
**Bonus:** Unter **Weitere Optionen** haben Sie die Möglichkeit, den Abruf von Passagen durch Auswahl des Optionsfeldees **Relevante Passagen einbeziehen** zu aktivieren. Passagen sind kurze, relevante und aus den vollständigen Dokumenten, die von Ihrer Abfrage zurückgegeben wurden, extrahierte Auszüge. Diese gewünschten Passagen werden aus den Feldern `text` der Dokumente in Ihrer Sammlung extrahiert. Weitere Informationen enthält der Abschnitt [Passagen](/docs/services/discovery/query-parameters.html#passages). Für die Sammlung '{{site.data.keyword.discoveryshort}} News' ist der Abruf von Passagen nicht verfügbar.
