...
-  Dieses Lernprogramm geht davon aus, dass Sie Ihre Serviceberechtigungsnachweise kennen.
   ...
   -  Kopieren Sie die Werte für den Benutzernamen (`username`) und das Kennwort (`password`) und vergewissern Sie sich, dass der Wert für die `URL` mit dem in den folgenden Beispielen verwendeten Wert übereinstimmt. Falls nicht, ersetzen Sie ihn ebenfalls.
...
1.  Erstellen Sie eine Umgebung.
    ...
    ```bash
    curl -X POST -u "{benutzername}":"{kennwort}" -H "Content-Type: application/json" -d '{ "name": "my_environment", "description": "My environment" }' "https://gateway.watsonplatform.net/discovery/api/v1/environments?version=2017-11-07"
    ```
    {: pre}
...
1. Erstellen Sie eine Sammlung.
    ...
    ```bash
    curl -X POST -u "{benutzername}":"{kennwort}" -H "Content-Type: application/json" -d '{ "name": "test_collection", "description": "My test collection", "configuration_id": "{konfigurations-id}", "language_code": "en" }' "https://gateway.watsonplatform.net/discovery/api/v1/environments/{umgebungs-id}/collections?version=2017-11-07"
    ```
    {: pre}
...
1.  Fügen Sie die Dokumente hinzu, die durchsucht werden sollen.
    ...
    1.  Geben Sie den folgenden Befehl aus, um die Daten von `cranfield-data-json` in die Sammlung `cranfield_collection` hochzuladen. Ersetzen Sie hierbei `{benutzername}`, `{kennwort}`, `{pfad_zur_datei}`, `{umgebungs-id}` und `{sammlungs-id}` durch die für Sie geltenden Angaben. Bitte beachten Sie, dass es weitere Optionen gibt (-d für das Debug und –v für eine ausführliche Ausgabe von curl).

        ```bash
        python ./disco-upload.py -u {benutzername}:{kennwort} -i {pfad_zur_datei}/cranfield-data.json –e {umgebungs-id} -c {sammlungs-id}
        ```
        {: pre}
...
1.  Führen Sie das Script für den Trainingsdatenupload aus, um die Trainingsdaten in {{site.data.keyword.discoveryshort}} hochzuladen. Ersetzen Sie hierbei `{benutzername}`, `{kennwort}`, `{pfad_zur_datei}`, `{umgebungs-id}` und `{sammlungs-id}` durch die für Sie geltenden Angaben. Bitte beachten Sie, dass es weitere Optionen gibt (`-d` für das Debug und `–v` für eine ausführliche Ausgabe von curl).

    ```bash
    python ./disco-train.py -u {benutzername}:{kennwort} -i {path_to_file}/cranfield-gt.csv –e {umgebungs-id} -c {sammlungs-id}
    ```
    {: pre}
...
1.  Mit einem cURL-Befehl können Sie in Ihrer Sammlung nach Dokumenten suchen. Führen Sie eine Abfrage mit einem Aufruf der Abfrage-API aus, um nicht eingestufte Ergebnisse anzuzeigen. Ersetzen Sie hierbei `{benutzername}`, `{kennwort}`, `{umgebungs-id}` und `{sammlungs-id}` durch die für Sie geltenden Angaben. Die zurückgegebenen Ergebnisse sind nicht eingestuft und verwenden die {{site.data.keyword.discoveryshort}}-Standardeinstufungsformeln. Sie können weitere Abfragen ausprobieren, indem Sie die `CSV`-Datei mit den Trainingsdaten öffnen und den Wert der ersten Spalte in den Abfrageparameter kopieren.

    ```bash
    curl -u "{benutzername}":"{kennwort}""https://gateway.watsonplatform.net/discovery/api/v1/environments/{umgebungs-id}/collections/{sammlungs-id}/query?version=2017-11-07&query=what is the basic mechanism of the transonic aileron buzz"
    ```
    {: pre}

1.  Nun führen Sie eine Suche unter Verwendung des Modells aus, indem Sie den Parameter `natural_language_query` angeben. Vergewissern Sie sich zuvor, dass ein trainiertes Modell wie im vorherigen Abschnitt beschrieben verfügbar ist. Fügen Sie den folgenden Code in Ihre Konsole ein; ersetzen Sie hierbei `{benutzername}`, `{kennwort}`, `{umgebungs-id}` und `{sammlungs-id}` durch Ihre Werte.

    ```bash
    curl -u "{benutzername}":"{kennwort}""https://gateway.watsonplatform.net/discovery/api/v1/environments/{umgebungs-id}/collections/{sammlungs-id}/query?version=2017-11-07&natural_language_query=what is the basic mechanism of the transonic aileron buzz"
    ```
    {: pre}
