...
`{username}` 및 `{password}`를 서비스 인증 정보로 대체하십시오.
...
API가 환경 ID, 환경 상태 및 환경에서 사용 중인 스토리지의 크기와 같은 정보가 포함된 응답을 리턴합니다. 사용자 환경이 `ready` 상태가 될 때까지 다음 단계로 이동하지 마십시오. 환경을 작성 중일 때 상태에서 `status:pending`을 리턴하면 `GET /v1/environments/{environment_id}` 메소드를 사용하여 준비될 때까지 상태를 확인하십시오. 이 예에서 `{username}` 및 `{password}`를 서비스 인증 정보로 대체하고 `{environment_id}`를 환경을 작성할 때 리턴된 환경 ID로 대체하십시오.
