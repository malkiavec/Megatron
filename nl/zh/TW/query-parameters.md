...
查詢搜尋會依相關性順序傳回資料集的所有文件，內含完整強化和全文。查詢也會排除未提及查詢內容的任何文件。這些查詢是使用 [{{site.data.keyword.discoveryshort}} 查詢語言](/docs/services/discovery?topic=discovery-query-operators#query-operators)撰寫。
