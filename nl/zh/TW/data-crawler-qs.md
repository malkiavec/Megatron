...
API 傳回的回應包括諸如環境 ID、環境狀態以及環境使用的儲存空間量等資訊。在環境狀態變成 `ready` 之前，請不要進行下一步。當您建立環境時，如果狀態傳回 `status:pending`，請使用 `GET /v1/environments/{environment_id}` 方法來檢查狀態，直到它就緒。在此範例中，請將 `{username}` 和 `{password}` 取代為您的服務認證，並將 `{environment_id}` 取代為您建立環境時所傳回的環境 ID。
...
API 傳回的回應包括諸如集合 ID、集合狀態以及集合使用的儲存空間量等資訊。在集合狀態變成 `online` 之前，請不要進行下一步。當您建立集合時，如果狀態傳回 `status:pending`，請使用 `GET /v1/environments/{environment_id}/collections/{collection_id}` 方法來檢查狀態，直到它就緒。在此範例中，請將 `{username}` 和 `{password}` 取代為您的服務認證、將 `{environment_id}` 取代為您的環境 ID，並將 `{collection_id}` 取代為此步驟先前傳回的集合 ID。
