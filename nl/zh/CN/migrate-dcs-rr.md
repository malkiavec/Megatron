...
-  如果您培训了一个排名器并且需要迁移排名，那么应该获取从 {{site.data.keyword.documentconversionshort}} 返回的内容，然后将其摄入到 {{site.data.keyword.discoveryshort}} 中
-  如果没有任何培训数据要迁移，请使用[文档分段功能](/docs/services/discovery/building.html#doc-segmentation)将原始的源文档摄入到 {{site.data.keyword.discoveryshort}} 中
...
迁移结果之后，下一步是迁移已为内容创建的所有培训数据。有两个选项可用于迁移培训数据：从源 (`csv`) 迁移，以及从服务迁移。如果是通过 `csv` 文件上传的培训数据，并且仍有权访问该文件，那么应该从源进行迁移。如果使用的是 {{site.data.keyword.retrieveandrankshort}} 工具或无权访问原始 `csv` 文件，那么应该从服务进行迁移。
...
- 您需要有权访问初始用于上传培训信息的原始源 `csv` 文件。
- 您应确保对其建立索引的已培训文档的标识与在 {{site.data.keyword.retrieveandrankshort}} 中对其建立索引的已培训文档的标识一致。

如果可以满足所有迁移条件，那么建议您使用此方法将培训数据移至 {{site.data.keyword.discoveryshort}} 服务。

要迁移培训数据，请根据源数据的详情来修改[迁移教程](/docs/services/discovery/migrate-rnr-tut.html)中描述的过程。

### 从服务迁移培训数据
{: #extract-train}

要从 {{site.data.keyword.retrieveandrankshort}} 服务迁移培训数据，您需要执行以下操作：使用 {{site.data.keyword.retrieveandrankshort}} API 来抽取培训数据，将 {{site.data.keyword.retrieveandrankshort}} 培训 JSON 转换为 {{site.data.keyword.discoveryshort}} 可使用的格式，最后使用该 API 将培训数据摄入到 {{site.data.keyword.discoveryshort}} 中。

要从 {{site.data.keyword.retrieveandrankshort}} 中抽取培训数据，请使用 {{site.data.keyword.retrieveandrankshort}} 工具中的 `Export` 功能。成功下载完整导出后，解压缩保存的 `.zip` 文件。在该归档内有两个文件。培训数据存储在名为 `export-questions.json` 的文件中。此文件包含 JSON 培训对象的数组。

数组中的每个培训结果都按以下形式表示：

**样本 {{site.data.keyword.retrieveandrankshort}} 培训数据**
```json
{
...
{{site.data.keyword.discoveryshort}} 并不需要从 {{site.data.keyword.retrieveandrankshort}} 中导出的所有信息。以下片段显示 {{site.data.keyword.discoveryshort}} 培训条目的必需结构。
...
此时，您需要将 {{site.data.keyword.retrieveandrankshort}} 培训信息转换为 {{site.data.keyword.discoveryshort}} 培训信息。转换时请考虑以下几点。
...
- {{site.data.keyword.discoveryshort}} 不需要未回答的问题，将手动检查相关性培训的有效性。

![对迁移流排名](images/migrate-ranking.png)

例如，上面列出的**样本 {{site.data.keyword.retrieveandrankshort}} 培训数据**将转换为在 {{site.data.keyword.discoveryshort}} 工具中使用，如下所示：
