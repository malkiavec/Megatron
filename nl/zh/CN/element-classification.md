...
-  `parties` 数组，用于列出在输入文档中识别到的当事方以及当事方的角色、地址和联系人。
-  用于定义 `effective_dates`、`contract_amounts` 和 `termination_dates` 的数组。

此功能当前仅支持英语版本，请参阅[语言支持](/docs/services/discovery?topic=discovery-language-support#feature-support)以获取详细信息。
...
-  {{site.data.keyword.discoveryshort}} 工具包含名为 **Default Contract Configuration** 的配置，可用于扩充 PDF 文档集合。您还可以选择用于创建包含 `elements` 扩充项的定制配置。请参阅[集合需求](/docs/services/discovery?topic=discovery-element-classification#element-collection)以获取详细信息。
-  **轻量**套餐每月最多可以处理 500 个页面。
-  在**专用**环境中不可用。
-  使用“元素分类”时无法执行扩充后规范化。

**注：****Default Contract Configuration** 文件已于 2018 年 9 月 25 日更新。如果在此日期之前已将此配置应用于集合，请参阅[发行说明](/docs/services/discovery?topic=discovery-release-notes#25sept)以获取有关更新集合的信息。
...
选择工具中的 `Default Contract Configuration` 后，可以上传文档。如果您不熟悉如何创建集合和上传文档，请参阅[工具入门](/docs/services/discovery?topic=discovery-getting-started#getting-started)。
...
-  `text`：已分类元素的文本。
-  `types`：一个数组，包含零个或更多个 `label` 对象。每个 `label` 对象都包含 `nature` 字段和 `party` 字段；nature 字段用于列出元素对所识别到的当事方的影响（例如，`Right` 或 `Exclusion`），party 字段用于标识该元素影响的当事方。请参阅[了解合同解析](/docs/services/discovery?topic=discovery-contract_parsing#contract_parsing)中的[类型](/docs/services/discovery?topic=discovery-contract_parsing#contract_types)以获取其他信息。
-  `categories`：一个数组，包含零个或更多 `label` 对象。每个 `label` 对象的值列出了识别到的元素所属的功能类别。请参阅[了解合同解析](/docs/services/discovery?topic=discovery-contract_parsing#contract_parsing)中的[类别](/docs/services/discovery?topic=discovery-contract_parsing#contract_categories)以获取其他信息。
-  `attributes`：一个数组，用于列出定义元素属性的零个或更多个对象。当前支持的属性类型包括 `Location`（元素引用的地理位置）、`DateTime`（元素指定的日期、时间、日期范围或时间范围）和 `Currency`（货币值和单位）。`attributes` 数组中的每个对象还包含所识别到的元素的文本和位置；位置由输入文档中的文本的 `begin` 和 `end` 索引定义。请参阅[解析合同](/docs/services/discovery?topic=discovery-contract_parsing#contract_parsing)中的[属性](/docs/services/discovery?topic=discovery-contract_parsing#attributes)以获取其他信息。
...
某些语句不属于任何类型或类别，在这种情况下，服务会将 `types` 和 `categories` 数组作为空对象返回。
{: note}

某些语句涵盖多个主题，在这种情况下，服务会返回多组 `types` 和 `categories` 对象。
{: note}

某些语句不包含任何可识别的属性，在这种情况下，服务会将 `attributes` 数组作为空对象返回。
{: note}
