...
I dati vengono etichettati aggiungendo un `customer_id` di tua scelta (vedi le limitazioni in [Come etichettare i dati](/docs/services/discovery/information-security.html#labeling)) all'intestazione `X-Watson-Metadata` facoltativa. {{site.data.keyword.discoveryshort}} può quindi eliminarli per il `customer_id`.
...
Se tutti i documenti sono già stati indicizzati con il Data Crawler, dovrai rieseguirne l'indicizzazione per aggiungere l'intestazione `X-Watson-Metadata` e il `customer_id`.

1. Aggiorna la tua configurazione dell'adattatore di output del Data Crawler {{site.data.keyword.discoveryshort}} per includere il `customer_id `. Consulta [Configurazione dell'adattatore di output](/docs/services/discovery/data-crawler-discovery.html#output-adapter).
1. Pianifica un'indicizzazione. I documenti vengono inoltrati a {{site.data.keyword.discoveryshort}} utilizzando l'intestazione `X-Watson-Metadata` e verranno etichettati con il `customer_id` configurato.
