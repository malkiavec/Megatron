...
Actualmente, esta característica beta solo está soportada en inglés, consulte [Soporte de idiomas](/docs/services/discovery/language-support.html#feature-support) para obtener detalles.
...
**Nota:** La [Similaridad de entidad](/docs/services/discovery/building-kg.html#similarity), la [Evidencia](/docs/services/discovery/building-kg.html#evidence) y la [Canonicalización y filtrado](/docs/services/discovery/building-kg.html#canonicalization) están disponibles en todas las colecciones. En las recopilaciones creadas antes del `03-05-2018`, debe volver a ingerir los documentos para utilizar estas características.
...
El release beta de la consulta de entidades ofrece soporte a la entidad con base al contexto [disambiguation](/docs/services/discovery/building-kg.html#disambiguation) y a las consultas de [similarity] (/docs/services/discovery/building-kg.html#similarity). Una consulta de entidades de Knowledge Graph se realiza con solicitudes `POST` a objetos `JSON` para el punto final `v1/environments/{environment_id}/collections/{collection_id}/query_entities`.
...
-  `"feature": string` _required_ - the entity query feature to be used. Las características soportadas son: [disambiguate] (/docs/services/discovery/building-kg.html#disambiguation) y [similar_entities](/docs/services/discovery/building-kg.html#similarity).
...
-  `"evidence_count": INT` _optional_ El número de instancias de evidencia que deben volver en cada entidad identificada. El valor predeterminado es `0`. El valor máximo del campo `evidence_count` es 10.000 dividido entre el número especificado en el campo `count`. Consulte la sección [Evidence](/docs/services/discovery/building-kg.html#evidence) de esta página para obtener una descripción detallada y ejemplos.
