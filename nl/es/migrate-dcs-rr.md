...
| Sinónimos personalizados| Sí|  |
**Nota:** Esta tabla se actualizará a medida que se añadan nuevas funcionalidades de {{site.data.keyword.discoveryshort}}. Antes de empezar la migración propiamente dicha, primero debe [evaluar](#evaluate) los datos almacenados en su servicio {{site.data.keyword.retrieveandrankshort}} y entender cómo se trasladarán los distintos componentes que forman su solución actual. 
...
Consulte la [tabla de soporte de idioma nacional para {{site.data.keyword.discoveryshort}}](/docs/services/discovery/language-support.html). Las características de {{site.data.keyword.retrieveandrankshort}} están soportadas fundamentalmente mediante un soporte de idioma de {{site.data.keyword.discoveryshort}} **Básico**. 
