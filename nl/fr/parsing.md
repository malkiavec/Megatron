...
Le tableau `parties` spécifie les participants répertoriés dans le contrat. Chaque objet `party` identifié répertorie les parties prenantes par non et est mis en corrélation avec un `role` qui classifie le rôle de l'objet `party`. Les valeurs de `role` pouvant être renvoyées pour des contrats incluent, mais ne se limitent pas à :
